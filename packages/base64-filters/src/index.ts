 ...
 */
export const areFiltersSingleValue = (filters: Filters) => {
 ...
 */
const isLengthSingleValue = (filters: number, bytes: number) => bytes <= filters * 2;
 ...
 * @param bytes The bytes to check
 */
export const areBytesSingleValue = (filters: Filters, bytes: Uint8Array) => {
 ...
 */
export const computeFiltersLength = (filters: Filters) => {
 ...
 * @param filters The filters to check
 */
export const areFiltersEmpty = (filters: Filters) => {
	// Check for no keys
 ...
 */
export const singleFiltersToBytes = (filters: Filters) => {
 ...
 */
export const multipleFiltersToBytes = (filters: Filters) => {
...
export const encodeFilters = (filters: Filters) => {
	...
	return base64ToUrlSafe(
		base64FromBytes(
			areFiltersSingleValue(filters)
 ...
 * @throws If a filter has more than one value or if a value is larger than 65535
 */
export const encodeSingleFilters = (filters: Filters) => {
	if (areFiltersEmpty(filters)) {
		return '';
	}
 ...
 * @throws If a filter has more than 255 values or if a value is larger than 65535
 */
export const encodeMultiFilters = (filters: Filters) => {
	if (areFiltersEmpty(filters)) {
		return '';
	}
 ...
 */
export const bytesToSingleFilters = <T extends Filters>(bytes: Uint8Array, filters: T) => {
	const keys: (keyof T)[] = Object.keys(filters);
	const values = new Uint16Array(bytes.buffer);
	keys.forEach((key, i) => {
		const value = values[i] || 0;
		filters[key] = (!value ? [] : [value]) as T[typeof key];
	});
};
 ...
 */
export const bytesToMultiFilters = <T extends Filters>(bytes: Uint8Array, filters: T) => {
	const keys: (keyof T)[] = Object.keys(filters);
	let i = 0;
	let k = 0;
	...
	while (i < bytes.length && k < keys.length) {
		...
		filters[key] = values as T[typeof key];
		k++;
	}
};
 ...
 */
const validateBytes = (bytes: Uint8Array) => {
	if (!bytes.length) {
 ...
 */
export const decodeFilters = <T extends Filters>(base64: string, filters: T) => {
	if (!base64) {
		return;
	}
 ...
 * @throws If there is an error decoding the filters
 */
export const decodeSingleFilters = <T extends Filters>(base64: string, filters: T) => {
	if (!base64) {
		return;
	}
	const bytes = base64ToBytes(base64FromUrlSafe(base64));
 ...
 * @throws If there is an error decoding the filters
 */
export const decodeMultiFilters = <T extends Filters>(base64: string, filters: T) => {
	if (!base64) {
		return;
	}
	const bytes = base64ToBytes(base64FromUrlSafe(base64));
